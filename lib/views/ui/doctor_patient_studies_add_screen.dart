...
import 'package:medicare/controller/ui/doctor_patient_studies_add_controller.dart';
...
class DoctorPatientStudiesAddScreen extends StatefulWidget {
  const DoctorPatientStudiesAddScreen({super.key});

  @override
  State<DoctorPatientStudiesAddScreen> createState() => _DoctorPatientStudiesAddScreenState();
}

class _DoctorPatientStudiesAddScreenState extends State<DoctorPatientStudiesAddScreen> with UIMixin {
  DoctorPatientStudiesAddController controller = Get.put(DoctorPatientStudiesAddController());
  ...
  Widget build(BuildContext context) {
    return Layout(
      child: GetBuilder(
        init: controller,
        tag: 'doctor_patient_studies_add_controller',
        builder: (controller) {
                          ...
                          onTap: () {
                            controller.onRegister(instanceIndex).then((validationError) {
                              if (validationError != null) {
                                ...
                                controller.manager.getPatientStudies(controller.data[instanceIndex]!.selectedPatient!.userNumber, AuthService.loggedUserNumber);
                                if (context.mounted) {
                                  simpleSnackBar(context, "Estudio registrado con éxito", contentTheme.success);// Color(0xFF35639D));
                                }
                                controller.goToList(instanceIndex);
                              }
                            });
                          },
