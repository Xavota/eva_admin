...
class DoctorDatesListController extends MyController {
  final manager = DBManager.instance!;
  MyFormValidator basicValidator = MyFormValidator();

  DoctorModel? _loggedInDoctor;
  ...
  String getFullName(DateModel date) {
    return date.realPatient?.fullName?? date.tempPatient?.fullName?? "";
  }
  ...
  void updateDates() {
    if (_loggedInDoctor == null) return;

    manager.getDates(doctorOwnerID: _loggedInDoctor!.userNumber).then((datesInfo) {
      if (datesInfo == null) {
  ...
  void onInit() {
    ...
    _loggedInDoctor = AuthService.loggedUserData as DoctorModel?;
    updateDates();
