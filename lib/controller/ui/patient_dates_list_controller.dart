...
class PatientDatesListController extends MyController {
  ...
  /*GlobalKey addNewContentKey() {
    _contentKeys.add(GlobalKey());
    contentKey = _contentKeys.last;
    return contentKey;
  }

  void disposeContentKey(GlobalKey key) {
    if (_contentKeys.contains(key)) {
  ...
  void updateDates(int instanceIndex) {
    ...
    manager.getDates(userNumber: _loggedInPatient!.userNumber).then((datesInfo) {
      ...
      _dates = datesInfo;
      Debug.log("Updated Dates", overrideColor: Colors.white);
      doUpdate(instanceIndex, true);
    });
  }
